...
export default function Student(props) {
  ...
  const handleDoSubmit = (e) => {
    ...
    setSubmitted(true);
    axios.post("http://localhost:4000/data", formValues);
  };
  ...
  return (
    <div className="detail">
      {/* {Object.entries(formErrors).length == 0 && submitted && (
        <div> Details Submitted Sucessfully </div>
      )} */}
      ...
      <div>
           ...
           <h1>Details</h1>
            ...
            {countries.map((tableData) => {
              return (
                <div>
                 
                  <h2>FirstName: {tableData.fname}</h2>
                 <h2> LastName: {tableData.lname}</h2>
                 <h2>Age:  {tableData.age}</h2>
                 <h2>Email:  {tableData.email}</h2>
                 <h2> Password: {tableData.password}</h2>
                 <h2> Gender: {tableData.Gender}</h2>
                 <h2>Country: {tableData.country}</h2>
                  </div> 
              );
            })}
